module ModSpox
  module Plugins
    class Triggers < ::ModSpox::Plugin
      ...
      # }
      def register(m)
        ::ModSpox::Logger.debug "Received trigger registration hash for regex: #{m[:regexp]}"
        if(m[:trigger])
          @triggers << m[:trigger]
          @triggers.uniq!
      ...
      def parse(message)
        ...
        ::ModSpox::Logger.debug "Parsing message: #{message}"
        @triggers.each do |trigger|
          if(m[0, trigger.length] == trigger)
            ::ModSpox::Logger.debug "Trigger match on: #{trigger}"
            m.slice!(0, trigger.length)
            @signatures.each do |sig|
              ::ModSpox::Logger.debug "Checking message against signature: #{sig[:regexp]}"
              args = m.scan(sig[:regexp])
              unless(args.empty?)
                args = args[0]
                ::ModSpox::Logger.debug "Signature match on: #{sig[:regexp]}"
                args = Hash[sig[:matches].zip(args)] if sig[:matches]
                @pool.process do
                  if(sig[:send])
